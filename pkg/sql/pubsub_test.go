...
func newPgx(t *testing.T) sql.Beginner {
	connStr := os.Getenv("WATERMILL_TEST_POSTGRES_HOST")
	if connStr == "" {
		connStr = "postgres://watermill:password@localhost/watermill?sslmode=disable"
	}
...
func TestPgxPostgreSQLQueue(t *testing.T) {
	...
	tests.TestPubSub(
		...
		func(t *testing.T) (message.Publisher, message.Subscriber) {
			return createPostgreSQLQueue(t, newPgx(t))
		},
		nil,
	)
}
...
func TestNotMissingMessages(t *testing.T) {
	...
	tx0, err := db.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.ReadCommitted})
	assert.NoError(t, err)
	time.Sleep(time.Millisecond * 10)

	tx1, err := db.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.ReadCommitted})
	assert.NoError(t, err)
	time.Sleep(time.Millisecond * 10)

	txRollback, err := db.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.ReadCommitted})
	assert.NoError(t, err)
	time.Sleep(time.Millisecond * 10)

	tx2, err := db.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.ReadCommitted})
	assert.NoError(t, err)
	time.Sleep(time.Millisecond * 10)
